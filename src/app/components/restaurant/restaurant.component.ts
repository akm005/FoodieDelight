...
export class RestaurantComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      ...
      contact: [data?.contact ?? '', [Validators.required,Validators.pattern(/^\d{3}-\d{4}-\d{3}$/)]],
      address: [data?.address ?? '', [Validators.required]],
      city: [data?.city ?? '', [Validators.required]],
      state: [data?.state ?? '', [Validators.required]],
      zip: [data?.zip ?? '', [Validators.required,Validators.pattern(/^\d{6}$/)]],
      open: [data?.open ?? '', [Validators.required]],
      close: [data?.close ?? '', [Validators.required]],
      type: [data?.type ?? '', [Validators.required,Validators.pattern(/^(VEG|NVEG|VEGNV|VG|JN)$/)]],
